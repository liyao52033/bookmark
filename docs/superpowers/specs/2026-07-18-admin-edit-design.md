...
1. 用 **GitHub Token 即登录**（与插件同一鉴权模型）
2. 编辑模式支持：增、删、改、分类内拖动排序
3. **本地草稿 + 显式提交**：任意次操作只改内存，一次 PUT 一个 commit
4. 未登录访客体验与现网一致
...
2. 200：解析 JSON，设为 `bookmarksData`，保存 `fileSha`，进入编辑模式
3. 非 200：展示错误，不进入编辑模式

### 持久化

- `localStorage` 键：`bookmarkAdmin.repo`、`bookmarkAdmin.token`、`bookmarkAdmin.jsonPath`
- 下次打开「管理」时**仅预填**表单，**不**自动进入编辑模式（需再次点登录校验）
- **Token 绝不写入仓库、不出现在 commit 内容中**

### 退出

- 「退出」：若 `dirty`，先确认是否丢弃；关闭编辑 UI，恢复访客渲染（重新 `fetch('bookmarks.json')` 或使用登录时快照）；保留 localStorage 配置供下次预填
- 「丢弃更改」：用登录成功时保存的 `serverSnapshot` 深拷贝覆盖 `bookmarksData`，`dirty = false`，重新渲染（不重新请求也可；若用户明确需要最新远程则点退出再登录）
...
1. 错误 Token 无法进入编辑模式
2. 正确 Token 进入后，增/删/改/排序后 GitHub 上无新 commit
3. 点提交后仓库 `bookmarks.json` 一次更新，内容与界面一致
4. 丢弃更改后界面回到提交前状态
5. 访客模式 UI 与改前一致
6. 同分类拖动顺序写入 JSON 数组顺序正确
7. 提交冲突（模拟旧 sha）有明确错误提示

## 成功标准

- 管理员可用 Token 登录并完成书签 CRUD + 同分类排序
